...
export function usePolls_CreatePoll() {
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      ...
      const fn = (contracts as any).polls_createPoll;
...
export function usePolls_Vote() {
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).polls_vote;
...
export function usePolls_GetPoll() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).polls_getPoll;
...
export function usePolls_GetPollCount() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).polls_getPollCount;
...
export function usePolls_HasAccountVoted() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).polls_hasAccountVoted;
...
export function usePolls_IsPollOpen() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).polls_isPollOpen;

...
import { usePolls_CreatePoll } from './hooks';



import { usePolls_Vote } from './hooks';



import { usePolls_GetPoll } from './hooks';



import { usePolls_GetPollCount } from './hooks';



import { usePolls_HasAccountVoted } from './hooks';



import { usePolls_IsPollOpen } from './hooks';
...
export default function DebugContracts() {
  ...
  const [openContract, setOpenContract] = useState<string | null>("polls");
    ...
    <div style={S.page}>
      ...
      <div style={S.panel}>
        <button 
          style={S.panelBtn} 
          onClick={() => {
            setOpenContract(openContract === "polls" ? null : "polls");
        ...
        >
          <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
            ...
            <span style={S.panelName}>Polls</span>
            <span style={S.panelExt}>.clar</span>
          </div>
          <span style={S.headerTag}>{openContract === "polls" ? '▲' : '▼'}</span>
        </button>
        ...
        {openContract === "polls" && (
          <div style={Object.assign({ padding: '20px', width:'100%' })} className='space-y-4'>
                ...
                {activeTab === 'write' && (
                  <FunctionCard_Polls_CreatePoll 
                    num="00"
                    isOpen={openFunction === "polls_create-poll"}
                    onToggle={() => setOpenFunction(openFunction === "polls_create-poll" ? null : "polls_create-poll")}
                  />
                )}
                ...
                {activeTab === 'write' && (
                  <FunctionCard_Polls_Vote 
                    num="01"
                    isOpen={openFunction === "polls_vote"}
                    onToggle={() => setOpenFunction(openFunction === "polls_vote" ? null : "polls_vote")}
                  />
                )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Polls_GetPoll 
      num="02"
      isOpen={openFunction === "polls_get-poll"}
      onToggle={() => setOpenFunction(openFunction === "polls_get-poll" ? null : "polls_get-poll")}
    />
  )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Polls_GetPollCount 
      num="03"
      isOpen={openFunction === "polls_get-poll-count"}
      onToggle={() => setOpenFunction(openFunction === "polls_get-poll-count" ? null : "polls_get-poll-count")}
    />
  )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Polls_HasAccountVoted 
      num="04"
      isOpen={openFunction === "polls_has-account-voted"}
      onToggle={() => setOpenFunction(openFunction === "polls_has-account-voted" ? null : "polls_has-account-voted")}
    />
  )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Polls_IsPollOpen 
      num="05"
      isOpen={openFunction === "polls_is-poll-open"}
      onToggle={() => setOpenFunction(openFunction === "polls_is-poll-open" ? null : "polls_is-poll-open")}
    />
  )}
...
function FunctionCard_Polls_CreatePoll({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = usePolls_CreatePoll();
  const isReadOnly = false;

  
  const [args, setArgs] = useState<Record<string, string>>({
    'question': '', 'option-1': '', 'option-2': '', 'option-3': '', 'duration-blocks': '', 
  });
    ...
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          ...
          <span style={S.fnName}>CREATE-POLL</span>
        </div>
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['question'] ?? '', {"string-ascii":{"length":280}}, 'question'),toClarityValue(args['option-1'] ?? '', {"string-ascii":{"length":64}}, 'option-1'),toClarityValue(args['option-2'] ?? '', {"string-ascii":{"length":64}}, 'option-2'),toClarityValue(args['option-3'] ?? '', {"string-ascii":{"length":64}}, 'option-3'),toClarityValue(args['duration-blocks'] ?? '', "uint128", 'duration-blocks'),]); }}>
          
            
            <div>
              <label style={S.argLabel}>question <span style={S.argType}>STRING-ASCII</span></label>
              
              <input style={S.argInput} value={args['question']} onChange={e => setArg('question', e.target.value)} />
              
            </div>
            
            <div>
              <label style={S.argLabel}>option-1 <span style={S.argType}>STRING-ASCII</span></label>
              
              <input style={S.argInput} value={args['option-1']} onChange={e => setArg('option-1', e.target.value)} />
              
            </div>
            
            <div>
              <label style={S.argLabel}>option-2 <span style={S.argType}>STRING-ASCII</span></label>
              
              <input style={S.argInput} value={args['option-2']} onChange={e => setArg('option-2', e.target.value)} />
              
            </div>
            
            <div>
              <label style={S.argLabel}>option-3 <span style={S.argType}>STRING-ASCII</span></label>
              
              <input style={S.argInput} value={args['option-3']} onChange={e => setArg('option-3', e.target.value)} />
...
function FunctionCard_Polls_Vote({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = usePolls_Vote();
  const isReadOnly = false;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['poll-id'] ?? '', "uint128", 'poll-id'),toClarityValue(args['option'] ?? '', "uint128", 'option'),]); }}>
          
            
            <div>
              <label style={S.argLabel}>poll-id <span style={S.argType}>UINT128</span></label>
              
              <input style={S.argInput} value={args['poll-id']} onChange={e => setArg('poll-id', e.target.value)} />
              
            </div>
            
            <div>
              <label style={S.argLabel}>option <span style={S.argType}>UINT128</span></label>
              
              <input style={S.argInput} value={args['option']} onChange={e => setArg('option', e.target.value)} />
...
function FunctionCard_Polls_GetPoll({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = usePolls_GetPoll();
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>GET-POLL</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['poll-id'] ?? '', "uint128", 'poll-id'),]); }}>
...
function FunctionCard_Polls_GetPollCount({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = usePolls_GetPollCount();
  const isReadOnly = true;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>GET-POLL-COUNT</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
...
function FunctionCard_Polls_HasAccountVoted({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = usePolls_HasAccountVoted();
  const isReadOnly = true;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>HAS-ACCOUNT-VOTED</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['poll-id'] ?? '', "uint128", 'poll-id'),toClarityValue(args['voter'] ?? '', "principal", 'voter'),]); }}>
            ...
            <div>
              <label style={S.argLabel}>voter <span style={S.argType}>PRINCIPAL</span></label>
              
              <input style={S.argInput} value={args['voter']} onChange={e => setArg('voter', e.target.value)} />
...
function FunctionCard_Polls_IsPollOpen({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = usePolls_IsPollOpen();
  const isReadOnly = true;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>IS-POLL-OPEN</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
